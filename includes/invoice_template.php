...
function getLuxuryInvoiceHTML($orderId, $orderData, $items, $isEmail = true) {
    ...
    $customerName = $orderData['name'] ?? 'Valued Customer';
    $phone = $orderData['phone'] ?? 'N/A';
    $address = $orderData['address'] ?? 'N/A';
    $paymentMethod = isset($orderData['payment_method']) ? strtoupper($orderData['payment_method']) : 'COD';
    ...
    foreach ($items as $item) {
        ...
        $pName = htmlspecialchars($item['name'] ?? $item['product_name'] ?? 'Product');
    ...
    $invoiceContent = "
        <div style='max-width: 700px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.05); border: 1px solid #eee;'>
            ...
            <div style='background-color: $c_primary; padding: 40px; text-align: center; border-bottom: 4px solid $c_accent;'>
                ...
                <p style='color: #fff; margin: 10px 0 0; font-size: 11px; text-transform: uppercase; letter-spacing: 2px; opacity: 0.7;'>Event Souvenirs & Fashion</p>
            </div>
            ...
            <div style='padding: 40px;'>
                ...
                <table style='width: 100%; margin-bottom: 40px;'>
                    <tr>
                        <td style='vertical-align: top;'>
                            <h4 style='margin: 0 0 5px; color: $c_gray; font-size: 11px; text-transform: uppercase; letter-spacing: 1px;'>Billed To</h4>
                        ...
                        <td style='vertical-align: top; text-align: right;'>
                            <h4 style='margin: 0 0 5px; color: $c_gray; font-size: 11px; text-transform: uppercase; letter-spacing: 1px;'>Invoice Info</h4>
                            <h3 style='margin: 0; color: $c_primary; font-size: 18px;'>#$orderId</h3>
                            <p style='margin: 5px 0 0; color: $c_gray; font-size: 14px; line-height: 1.5;'>Date: $date<br>Method: <span style='color: $c_primary; font-weight: bold;'>$paymentMethod</span></p>
                        </td>
                    </tr>
                </table>
                ...
                <table style='width: 100%; border-collapse: collapse; margin-bottom: 30px; font-size: 14px;'>
                    <thead>
                        <tr style='background-color: $c_bg;'>
                            <th style='padding: 12px 10px; text-align: left; font-size: 11px; text-transform: uppercase; color: $c_gray; letter-spacing: 1px;'>Item Description</th>
                            <th style='padding: 12px 10px; text-align: center; font-size: 11px; text-transform: uppercase; color: $c_gray; letter-spacing: 1px;'>Qty</th>
                            <th style='padding: 12px 10px; text-align: right; font-size: 11px; text-transform: uppercase; color: $c_gray; letter-spacing: 1px;'>Price</th>
                            <th style='padding: 12px 10px; text-align: right; font-size: 11px; text-transform: uppercase; color: $c_gray; letter-spacing: 1px;'>Total</th>
                        </tr>
                    </thead>
                    <tbody>
                        $itemsHtml
                ...
                <div style='border-top: 2px solid $c_primary; padding-top: 20px;'>
                    <table style='width: 100%; max-width: 280px; margin-left: auto;'>
                        <tr>
                            <td style='padding: 5px 0; color: $c_gray; font-size: 14px;'>Subtotal:</td>
                        ...
                        <tr>
                            <td style='padding: 5px 0; color: $c_gray; font-size: 14px;'>Shipping:</td>
                            <td style='padding: 5px 0; text-align: right; color: #198754; font-size: 14px;'>Free Shipping</td>
                        </tr>
                        <tr>
                            <td style='padding: 15px 0; font-size: 16px; font-weight: bold; color: $c_primary;'>Total:</td>
                ...
                <div style='margin-top: 50px; text-align: center; color: $c_gray; font-size: 12px; border-top: 1px solid #eee; padding-top: 20px;'>
                    <p style='margin-bottom: 8px;'>Thank you for choosing <strong>{$_ENV['APP_NAME']}</strong>.</p>
                    <p style='margin: 0;'>Need help? Contact us at <a href='mailto:{$_ENV['EMAIL_ADMIN']}' style='color: $c_accent; text-decoration: none;'>{$_ENV['EMAIL_ADMIN']}</a> or {$_ENV['PHONE_ADMIN']}</p>
                </div>
            </div>
        </div>
    ";
